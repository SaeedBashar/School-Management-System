...
Public Class AdminAnnouncementView
    ...
    Protected Sub delann_ServerClick(sender As Object, e As EventArgs)
        Dim cmd As New SqlCommand
        Dim query As String = "delete from Announcement where title = '" & anninp.Value & "'"
        cmd.CommandText = query
        cmd.Connection = Con

        Con.Open()
        If cmd.ExecuteNonQuery <> 0 Then
            ClientScript.RegisterClientScriptBlock(Me.GetType(), "success", "<script>swal('Announcement deleted Successfully!!')</script>")
        Else
